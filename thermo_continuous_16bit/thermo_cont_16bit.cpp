...
void median(Container& c) {
    using T = typename Container::value_type;

    // using C_type = typename std::decay<Container>::type;
    // using T = typename std::remove_pointer<C_type>::type;
    std::sort(std::begin(c), std::end(c), std::greater<T>{});
}
...
float decode_16bits(const char* buf) {
    size_t data = (buf[0] << 8 | buf[1]);

    return static_cast<float>((data) / 128);
}
...
int main(int argc, char const* argv[])
{
    ...
    // 2    : Continuous mode
    // 1-0  : 0
    char cmd_16bits[2] = { 0b00001000, 0b10000000 };//0x0C, 0x80
    bcm2835_spi_transfern(cmd_16bits, 2);
    ...
    for (size_t j = 0; j < 7; j++)
    {
        ...
        while (temps.size() < 8)
        {
            ...
            if (!std::all_of(std::begin(raw_temps), std::end(raw_temps), [](char a) {return a == 0;}))
                temps.push_back(decode_16bits(raw_temps));
        }
